...
# WS11.T intervention flag (see compute_assembly_stats).
_WS11T_SKIP_ASSEMBLY_STATS = True
...
def compute_assembly_stats(
    ...
    # ===================== WS11.T INTERVENTION ARM =========================
    # The ONLY modification to the archived V3 code tree (git 471eb28).
    # It short-circuits the 19 genuine assembly statistics -- the 11 contig
    # length stats, the 4 base-composition stats and the 4 distributional
    # stats -- which are exactly the features V4/V5 deleted.  Everything else
    # is untouched: same FASTA reader, same k-mer counter, same 26-wide
    # feature vector, same normalizer, same magicc_v3.onnx session.
    # features[0:19] are left at 0.0; features[19:26] (log10_total_kmer_count
    # plus the 6 k-mer summary features that V5 retains) are computed below by
    # the ORIGINAL code.
    # PREDICTIONS FROM THIS ARM ARE MEANINGLESS -- it is a timing probe only.
    if _WS11T_SKIP_ASSEMBLY_STATS:
        features[19] = log10_total_kmer_count
        if kmer_counts is not None:
            total_kmer_sum = float(kmer_counts.sum())
            features[20] = total_kmer_sum
            unique_count = int(np.count_nonzero(kmer_counts))
            features[21] = float(unique_count)
            duplicate_count = int(np.sum(kmer_counts > 1))
            features[22] = float(duplicate_count)
            if total_kmer_sum > 0:
                probs = kmer_counts.astype(np.float64) / total_kmer_sum
                nonzero_mask = probs > 0
                features[23] = -float(np.sum(probs[nonzero_mask]
                                             * np.log2(probs[nonzero_mask])))
            n_total_kmers = len(kmer_counts)
            features[24] = unique_count / n_total_kmers if n_total_kmers > 0 else 0.0
            features[25] = duplicate_count / unique_count if unique_count > 0 else 0.0
        return features
    # =================== END WS11.T INTERVENTION ARM =======================
