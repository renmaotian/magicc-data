...
1,407 files, 136 MB — including **all three** taxonomic-holdout evaluations
(leave-phylum-out, leave-family-out and leave-genus-out).
[`results/README.md`](results/README.md) is the inventory:
...
| The **three** taxonomic-holdout models (`magicc_holdout_{phylum,family,genus}.onnx`, 162–170 MB each) | validation artefacts, not released models, and promised by neither availability statement. **Their evaluation outputs are deposited in full** — `results/holdout_{phylum,family,genus}/` — only the weights are not. Each is regenerated by the deposited training scripts from the deposited splits, and for the genus model the SHA256 and the ONNX-versus-PyTorch equivalence check are deposited so a regenerated copy can be checked against the one used |
