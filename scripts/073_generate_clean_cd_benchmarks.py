#!/usr/bin/env python3
"""
WS1.2 / WS1.3 (+ WS7.7) — Generate the leakage-free benchmark Sets C_clean and D_clean.
...
sampled quantities:
  1. per-simulation seeds (see SEEDS below) instead of one `SEED + idx` stream, so that
     every one of the 1,000 samples is independently reproducible (R1-m15);
  2. full per-sample provenance is captured and written to generation_metadata.tsv
     (contaminant accessions, targets, observed values, tiers, dropout parameters);
  3. a final constraint guard that trims contaminant contigs if and only if
     contaminant_bp > dominant_retained_bp, which the V5 training data also enforces.
     The guard is counted and reported; it is a no-op on every sample where the
     inherited logic already satisfies the constraint.

DESIGN
------
  100 references x 10 independent simulations = 1,000 genomes per set
  target completeness  ~ Uniform[0.50, 1.00)  (drawn from the per-set design RNG)
  target contamination ~ Uniform[0, 100)      (drawn from the per-set design RNG)
  contaminants         : 1-5 genomes, cross-phylum, drawn ONLY from
                         data/splits/test_genomes.tsv, excluding the dominant's phylum
  constraint           : contaminant_bp <= dominant_retained_bp, i.e.
                         contamination% <= completeness% (both share the dominant's full
                         reference length as denominator); contamination <= 100%

SEEDS (fully deterministic)
---------------------------
  design RNG per set   : np.random.default_rng(SET_BASE)         -> 1,000 target
                         completeness and 1,000 target contamination values
  per-sample RNG       : np.random.default_rng(SET_BASE + 1000 * ref_index + replicate)
  SET_BASE             : C_clean = 7_300_000,  D_clean = 7_400_000
  genome index         : idx = 10 * ref_index + replicate  -> genome_{idx}

OUTPUTS (per set, in data/benchmarks/set_{C,D}_clean/)
-----------------------------------------------------
  fasta/genome_<idx>.fasta
  metadata.tsv               same 8 columns as the superseded sets, plus ref_index/replicate
  labels.npy                 float32 [n, 2] = (true_completeness, true_contamination)
  generation_metadata.tsv    full per-sample provenance (also satisfies WS7.7)
...
N_REFS = 100
N_SIMS = 10
SET_BASE = {'C': 7_300_000, 'D': 7_400_000}

# Module defaults reproduced here purely so they can be RECORDED per sample.
# (magicc.fragmentation.apply_coverage_dropout / apply_gc_biased_loss /
#  apply_repeat_exclusion signatures — verified by inspection, not overridden.)
DROPOUT_PARAMS = {
    'coverage_mean': 30.0,
    'coverage_sigma': 1.0,
...
def _init_worker(test_df_path):
    """Load the contaminant pool once per worker.

    Behaviourally identical to scripts/25's per-call `pd.read_csv(test_df_path)`:
    same file, same parser, same row order, therefore the same
    `candidates.sample(random_state=...)` result.
    """
    global _TEST_DF
...
def get_cross_phylum_contaminants(dominant_phylum, test_df, rng, n_contaminants=1):
    ...
    if len(candidates) == 0:
        candidates = test_df                                    # fallback: any genome
...
def generate_clean_cd_genome(args):
    ...
    try:
        ...
        if target_contamination <= 0:
            ...
            n_cont_requested = int(rng.integers(1, 6))     # 1-5 cross-phylum genomes
            ...
            if len(contaminant_sequences) == 0:
                ...
                # Inherited explicit cap (scripts/25): contamination must not exceed
                # target by more than 20 % + 5 pp.
        ...
        # ---- NEW: hard constraint guard, contaminant_bp <= dominant_retained_bp -----
        ...
        # Inherited label arithmetic from scripts/25.
...
def build_tasks(set_name, refs, n_refs, n_sims, fasta_dir):
    base = SET_BASE[set_name]
    design_rng = np.random.default_rng(base)
    n_total = n_refs * n_sims
    comp_targets = design_rng.uniform(0.50, 1.0, size=n_total)
    cont_targets = design_rng.uniform(0.0, 100.0, size=n_total)
    ...
    for r in range(n_refs):
        ...
        for s in range(n_sims):
            idx = r * n_sims + s
            tasks.append((idx, r, s, row, float(comp_targets[idx]),
                          float(cont_targets[idx]), str(fasta_dir), set_name,
                          int(base + 1000 * r + s)))
    return tasks
...
def generate_set(set_name, n_refs, n_sims, out_dir):
    print('\n' + '=' * 78)
    print(f'Generating Set {set_name}_clean: {n_refs} refs x {n_sims} sims '
          f'= {n_refs * n_sims} genomes')
    print('=' * 78)
    ...
    refs = refs.iloc[:n_refs]

    fasta_dir = out_dir / 'fasta'
    fasta_dir.mkdir(parents=True, exist_ok=True)
    ckpt_path = out_dir / 'generation_checkpoint.jsonl'

    tasks = build_tasks(set_name, refs, n_refs, n_sims, fasta_dir)
    ...
    if todo:
        with open(ckpt_path, 'a') as ck, \
            ...
            for i, rec in enumerate(pool.imap_unordered(
                    generate_clean_cd_genome, todo, chunksize=1), 1):
                if rec.get('error'):
                    n_err += 1
    ...
    print(f'  generation: {time.time() - t0:.0f}s, {len(done)} complete, {n_err} errors')

    if len(done) != n_refs * n_sims:
        print(f'  WARNING: {len(done)} records for {n_refs * n_sims} expected samples')

    gen = pd.DataFrame([done[f'genome_{t[0]}'] for t in tasks
                        if f'genome_{t[0]}' in done])
    gen = gen.reindex(columns=GEN_COLS)
    ...
    meta = pd.DataFrame({
        ...
        'sample_type': f'set_{set_name.lower()}_clean',
...
# --------------------------------------------------------------------- validation
def validate_set(set_name, out_dir):
    print('\n' + '-' * 78)
    print(f'VALIDATION — set_{set_name}_clean')
    print('-' * 78)
    ...
    fasta_dir = out_dir / 'fasta'
    rep = {'set': f'set_{set_name}_clean', 'n_samples': int(len(meta))}

    # 1. consistency of the three output files
    ...
    # 2. FASTA integrity: exists, non-empty, parseable, bp matches recorded total_length
    ...
    # 3. constraint: contaminant_bp <= dominant_retained_bp  <=>  cont% <= comp%
    ...
    rep['violations_contamination_gt_100'] = int((gen['observed_contamination'] > 100).sum())
    ...
    rep['completeness'] = {'mean': round(float(comp.mean()), 3),
                           'std': round(float(comp.std()), 3),
                           'min': round(float(comp.min()), 3),
                           'max': round(float(comp.max()), 3),
                           'hist_50_100_5bins': comp_hist.tolist(),
                           'chi2_vs_uniform': round(
                               float(((comp_hist - exp) ** 2 / exp).sum()), 2)}
    rep['contamination'] = {'mean': round(float(cont.mean()), 3),
                            'std': round(float(cont.std()), 3),
                            'min': round(float(cont.min()), 3),
                            'max': round(float(cont.max()), 3),
                            'hist_0_100_5bins': cont_hist.tolist(),
                            'chi2_vs_uniform': round(
                                float(((cont_hist - exp) ** 2 / exp).sum()), 2)}
    # Kolmogorov-Smirnov against the *target* uniforms (the observed values are the
    # realised, bias-perturbed versions of the targets, so we test the targets and
    # report the observed-vs-target correlation separately).
    ...
    rep['pearson_target_vs_observed_completeness'] = round(float(
        np.corrcoef(gen['target_completeness'], gen['observed_completeness'])[0, 1]), 4)
    rep['pearson_target_vs_observed_contamination'] = round(float(
        np.corrcoef(gen['target_contamination'], gen['observed_contamination'])[0, 1]), 4)

    # 5. reference / replicate structure and contaminant provenance
    rep['n_unique_dominants'] = int(gen['dominant_accession'].nunique())
    rep['sims_per_reference'] = sorted(
        gen.groupby('ref_index').size().unique().tolist())
    rep['dominant_phylum_counts'] = gen['dominant_phylum'].value_counts().to_dict()
    same_phylum = 0
    all_cont = []
    for dp, ca, cp in zip(gen['dominant_phylum'], gen['contaminant_accessions'],
        ...
        if isinstance(cp, str) and cp:
            ps = cp.split(';')
            same_phylum += sum(1 for x in ps if x == dp)
    ...
    rep['n_contaminants_distribution'] = gen['n_contaminants'].value_counts().sort_index().to_dict()
...
def main():
    ...
    ap.add_argument('sets', nargs='*', default=['C', 'D'], choices=['C', 'D'])
    ap.add_argument('--test', action='store_true',
                    help='2 refs x 2 sims into set_{X}_clean_TEST (does not touch real dirs)')
    ap.add_argument('--validate-only', action='store_true')
    args = ap.parse_args()
    sets = args.sets or ['C', 'D']

    n_refs, n_sims = (2, 2) if args.test else (N_REFS, N_SIMS)
    reports = {}
    for s in sets:
        out_dir = BENCHMARK_DIR / (f'set_{s}_clean_TEST' if args.test
                                   else f'set_{s}_clean')
        out_dir.mkdir(parents=True, exist_ok=True)
        if not args.validate_only:
            generate_set(s, n_refs, n_sims, out_dir)
        reports[f'set_{s}_clean'] = validate_set(s, out_dir)

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    name = ('ws1_23_generation_validation_TEST.json' if args.test
            else 'ws1_23_generation_validation.json')
    with open(RESULTS_DIR / name, 'w') as f:
        json.dump({'generated_utc': datetime.now(timezone.utc).isoformat(),
                   'n_refs': n_refs, 'n_sims_per_ref': n_sims,
                   'n_workers': N_WORKERS,
                   'set_base_seeds': SET_BASE,
    ...
    ok = all(r['fasta_integrity_pass'] and r['constraints_pass']
             and r['metadata_matches_generation_metadata'] and r['labels_match_metadata']
