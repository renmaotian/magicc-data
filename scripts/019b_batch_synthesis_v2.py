...
def init_kmer_globals():
    ...
    # Warm up Numba JIT - k-mer counting
...
def process_single_sample(args):
    ...
    try:
        ...
        # Compute k-mer summary features (no contigs needed)
...
class BatchPlanner:
    ...
    def _plan_within_phylum(self, sid, seed, qt):
        ...
        # Cap contamination at completeness: contaminant_bp must not exceed dominant_actual_bp
...
def process_batch(batch_id, split, offset, genome_index, pool, n_workers, logger):
    ...
    # Post-synthesis verification: remove samples where contamination > completeness
    # or where values are out of valid ranges [50,100] for completeness, [0,100] for contamination
    verified_results = []
    n_contam_exceeds = 0
    n_out_of_range = 0
    for r in results:
        (sid, kc, af, comp, contam, phylum, stype, qt, acc, fl, nc) = r
        # Check: actual_contamination must not exceed actual_completeness
        if contam > comp:
            n_contam_exceeds += 1
            continue
        # Check: completeness in [50, 100], contamination in [0, 100]
        if comp < 50.0 or comp > 100.0 or contam < 0.0 or contam > 100.0:
            n_out_of_range += 1
            continue
        verified_results.append(r)
    n_removed = len(results) - len(verified_results)
    if n_removed > 0:
        logger.info(f"    Batch {batch_id}: Verification removed {n_removed} samples "
                    f"({n_contam_exceeds} contam>comp, {n_out_of_range} out-of-range), "
                    f"{len(verified_results)} passed")
