...
Usage:
    python scripts/074_provenance_audit.py
    python scripts/074_provenance_audit.py --no-manifest      # skip SHA256 of 2,000 FASTAs
"""
...
Generated {datetime.now(timezone.utc).isoformat()} by `scripts/074_provenance_audit.py`.
...
(`scripts/025_benchmark_generate.py`): *"Set C: ALL Patescibacteriota from
...
```bash
python scripts/072_select_clean_cd_refs.py            # WS1.1  reference selection
python scripts/073_generate_clean_cd_benchmarks.py C D  # WS1.2/1.3  generation
python scripts/074_provenance_audit.py                # WS1.4  this audit
python scripts/075_run_magicc_clean_cd.py             # WS1.5  MAGICC V5 inference
```
"""
    ...
    def ws15_block(old_name):
        if ws15 is None or old_name not in ws15.index:
            return ('\n(MAGICC V5 accuracy comparison not yet computed — run '
                    '`scripts/075_run_magicc_clean_cd.py`.)\n')
        r = ws15.loc[old_name]
        return f"""
...
generated by `scripts/074_provenance_audit.py`.
...
(`scripts/025_benchmark_generate.py` docstring). Verified counts over the
